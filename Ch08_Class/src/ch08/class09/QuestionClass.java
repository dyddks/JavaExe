...
public static void answer2(Scanner sc) {
	Resolve1_Restourant res = new Resolve1_Restourant();
	Resolve1_Order order = new Resolve1_Order();
	
	order.order();
	res.deliveryFood(order.getMenuNum());
}

// 3번 문제에 대한 풀이
public static void answer3(Scanner sc) {
	Mart mart = new Mart(20);
...
public static void answer4(Scanner sc) {
	Song song = new Song();
	song.setSongInfo("Dancing Queen", "ABBA", "Arrival", "Benny Andersson, Agnetha Faltskog, Anni-Frid Lyngstad, Bjorn Ulvaeus", 2001, 2);
	song.show();
}
